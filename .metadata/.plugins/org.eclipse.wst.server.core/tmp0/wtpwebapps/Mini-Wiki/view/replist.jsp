  ...
  <div class="container-fluid">
    ...
    <div class="collapse navbar-collapse" id="navbarSupportedContent">
      <ul class="navbar-nav me-auto mb-2 mb-lg-0">
        ...
        <li class="nav-item">
          <a class="nav-link disabled" href="#" tabindex="-1" aria-disabled="true">${ r.username.toString().substring(1, r.username.toString().length()-1) }</a>
        </li>
        </c:forEach>
        </c:if>
      </ul>
...
<table class="table">
  ...
  <tbody>
  <c:if test="${ rek != null }">
        <c:forEach items="${rek}" var="re">
    <tr>
     
      <td>${ re.Utilisateur.toString().substring(1, re.Utilisateur.toString().length()-1) }</td>
      <td>${ re.Page.toString().substring(1, re.Page.toString().length()-1) }</td>
      <td>${ re.Contenu.toString().substring(1, re.Contenu.toString().length()-1) }</td>
    </tr>
     </c:forEach>
     </c:if>
    <tbody>
